...
- **Advisory, never a gate.** Output always carries "Silence ≠ clean — treat as a tripwire,
  ...
  pi exit / empty output / failed head-checkout) that posts *no* review exits `2` so the check turns red (annotated
...
examples/
  ...
  grafana-dashboard.json # importable Grafana dashboard over the Loki events (metrics.py).
deploy/
...
| `MODEL` | `DEFAULT_MODEL` (`deepseek/deepseek-v4-flash-0731`) | OpenRouter model id (local: auto-discovered) |
...
| `TRIVIAL_GLOBS` | `**/*.md` | comma-separated globs that gate treats as prose (replaces the default, doesn't extend it) |
